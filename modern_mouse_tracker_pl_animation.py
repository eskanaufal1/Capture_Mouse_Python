...
class ModernMouseTracker:
    def __init__(self):
        ...
        self.bounce_effects = []  # List to store active bounce animations
        ...
        self.root.bind('<KeyPress-p>', lambda e: self.shortcut_toggle_plinko())
        self.root.bind('<KeyPress-P>', lambda e: self.shortcut_toggle_plinko())
        self.root.bind('<KeyPress-o>', lambda e: self.shortcut_toggle_coordinates())
        self.root.bind('<KeyPress-O>', lambda e: self.shortcut_toggle_coordinates())
    ...
    def create_modern_ui(self):
        ...
        self.plinko_button.pack(side='left', padx=5)
        ...
        self.coordinates_button.pack(side='left', padx=5)
    ...
    def toggle_plinko(self):
        """Toggle plinko animation on/off"""
        self.show_plinko = not self.show_plinko
        if self.show_plinko:
            self.plinko_button.config(text="Plinko: ON (P)", bg='#2ed573')
        else:
            self.plinko_button.config(text="Plinko: OFF (P)", bg='#ff4757')
            self.bounce_effects.clear()
            
    def toggle_coordinates(self):
        """Toggle coordinates display on/off"""
        self.show_coordinates = not self.show_coordinates
        if self.show_coordinates:
            self.coordinates_button.config(text="Coords: ON (O)", bg='#2ed573')
        else:
            self.coordinates_button.config(text="Coords: OFF (O)", bg='#ff4757')
            
    def setup_plinko_pins(self):
        """Initialize plinko pin positions in a grid pattern"""
        pins = []
        ...
        for row in range(rows):
            for col in range(cols):
                # Offset every other row for plinko pattern
                ...
                # Only add pins that are within screen bounds
    ...
    def check_plinko_collisions(self, x, y):
        """Check for collisions between pointer and plinko pins"""
        if len(self.plinko_pins) == 0:
            return
            
        # Calculate distances to all pins using numpy for efficiency
        ...
        for idx in collision_indices:
            ...
            self.create_bounce_effect(pin_x, pin_y)
            
    def create_bounce_effect(self, pin_x, pin_y):
        """Create a visual bounce effect at pin collision"""
        # Check if there's already a recent bounce effect at this pin
        current_time = time.time()
        for effect in self.bounce_effects:
            if (abs(effect['x'] - pin_x) < 10 and 
                abs(effect['y'] - pin_y) < 10 and 
                current_time - effect['start_time'] < 0.2):
                return  # Don't create duplicate effects too quickly
        
        # Create new bounce effect
        bounce_effect = {
            'x': pin_x,
            'y': pin_y,
            'radius': self.pin_radius,
            'max_radius': self.pin_radius * 3,
    ...
    def shortcut_toggle_plinko(self):
        """Toggle plinko via keyboard shortcut with feedback"""
        self.toggle_plinko()
        status = "ON" if self.show_plinko else "OFF"
        print(f"Keyboard shortcut: Plinko animation {status} (P)")
        
    def shortcut_toggle_coordinates(self):
        """Toggle coordinates via keyboard shortcut with feedback"""
        self.toggle_coordinates()
        status = "ON" if self.show_coordinates else "OFF"
        print(f"Keyboard shortcut: Coordinates display {status} (O)")
    ...
    def on_mouse_move(self, event):
        ...
        # Check for plinko pin collisions
        if self.show_plinko and self.current_x is not None and self.current_y is not None:
            self.check_plinko_collisions(x, y)
    ...
    def update_bounce_effects(self):
        """Update and clean up bounce effects"""
        current_time = time.time()
        active_effects = []
        ...
        for effect in self.bounce_effects:
            ...
            if progress <= 1.0:
                ...
                effect['color_intensity'] = int(255 * (1 - progress))
                active_effects.append(effect)
        
        self.bounce_effects = active_effects
        
    def draw_plinko_pins(self):
        """Draw all plinko pins and their bounce effects"""
        # Draw static pins
        ...
        for effect in self.bounce_effects:
            ...
            # Create gradient color from orange to red
            red = min(255, intensity + 100)
            green = max(0, intensity - 50)
            ...
            # Draw inner glow
            inner_radius = effect['radius'] * 0.6
            inner_color = f"#{min(255, red + 50):02x}{min(255, green + 100):02x}00"
            self.canvas.create_oval(
                effect['x'] - inner_radius, effect['y'] - inner_radius,
                effect['x'] + inner_radius, effect['y'] + inner_radius,
                outline=inner_color, width=2, fill=''
            )
    ...
    def draw_all(self):
        ...
        # Draw plinko pins first (background layer)
        if self.show_plinko:
            self.draw_plinko_pins()
        ...
        if self.show_coordinates:
            ...
            # Build coordinate text based on what's enabled
            ...
            self.canvas.create_rectangle(
                text_x - 5, text_y - 15, text_x + 270, text_y + 15,
    ...
    def run(self):
        ...
        print("Modern Mouse Tracker with Plinko Animation Started!")
        print("- Move mouse to see tracking and plinko collisions")
